...
class Plotter:
    ...
    def generate(self, lead_ins: int = 1, carry_ons: int = 1):
        ...
        actors = []
        plot = self._expand(conflict, root_transform, {"leadIns": lead_ins, "carryOns": carry_ons}, expand_id="root").replace('*', '')
    ...
    def _apply_names(self, text: str, actors: List[dict]) -> str:
        ...
        Parameters:text (str): The text containing character symbols.
                   actors (list): A list to store character details.
...
if __name__ == "__main__":
    ...
    plot = generator.generate(lead_ins=2, carry_ons=3)
